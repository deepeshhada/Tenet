...
class Error_plot(object):
    ...
    def get_best_valid_test_error(self):
        ...
        # this is just for getting best of test (not for the report) =========================
        best_test_hr_index    = np.argmax(self.test_hr_list)
        best_test_ndcg_index  = np.argmax(self.test_ndcg_list)
        best_test_map_index   = np.argmax(self.test_map_list)
        best_test_hr_test     = self.test_hr_list[best_test_hr_index]
        best_test_ndcg_test   = self.test_ndcg_list[best_test_ndcg_index]
        best_test_map_test    = self.test_map_list[best_test_map_index]
        # ================
        ...
        if self.save_flag == True:
            result_file_append = 'result.res'
            print("Path where results are stored.",self.path + result_file_append)
            with open(self.path + result_file_append,'a') as fout:
                fout.write('val_hr: {:.4f} test_hr: {:.4f} val_ndcg: {:.4f} test_ndcg: {:.4f} val_map: {:.4f} test_map: {:.4f} test_hr (test): {:.4f} test_ndcg (test): {:.4f}  test_map (test): {:.4f} params: {}'.format(best_valid_hr,best_test_hr,best_valid_ndcg,best_test_ndcg,best_valid_map,best_test_map,best_test_hr_test,best_test_ndcg_test,best_test_map_test,str(self.args))+'\n')

        return best_valid_hr_index,best_valid_ndcg_index,best_valid_map_index,best_valid_hr,best_valid_ndcg,best_valid_map,best_test_hr,best_test_ndcg,best_test_map,best_test_hr_test,best_test_ndcg_test,best_test_map_test
